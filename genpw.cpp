...
static const std::string_view alnums = "1234567890qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM";

std::string genpw(const std::string_view &data, const size_t retlen, const std::string_view addalphabet, uchar *salt, size_t saltlen)
{
    std::string res;
    std::vector <char> shuffleVec;
    uchar * hash = new uchar[retlen * 2];
    genhash((uchar*)data.data(), data.length(), hash, retlen * 2, salt, saltlen);
    size_t i = 0;
